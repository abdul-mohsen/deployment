...
[ -f scripts/sql/ifritah-schema.sql ] \
    && grep -q 'CREATE TABLE `user`' scripts/sql/ifritah-schema.sql \
    && PASS "bundled schema includes user table" \
    || FAIL "bundled schema is missing or incomplete"
[ -f scripts/sql/ifritah-migrations/0001_auth_tables.sql ] \
    && grep -q 'CREATE TABLE IF NOT EXISTS `sessions`' scripts/sql/ifritah-migrations/0001_auth_tables.sql \
    && PASS "bundled auth migration is present" \
    || FAIL "bundled auth migration is missing"
grep -q 'init-tenant-db.sh" "\$TENANT_NAME" --schema-only' scripts/create-tenant.sh \
