...
# don't have to remember to export them. Explicit env vars still win because
# `set -a; . file` only sets unset/overrides; we re-export current values
# afterwards to preserve any values the caller intentionally provided.
_caller_dokku_port="${DOKKU_PORT:-}"
_caller_dokku_hostname="${DOKKU_HOSTNAME:-}"
for _f in "${REPO_DIR}/install.env" "${REPO_DIR}/config.env"; do
  if [ -f "$_f" ]; then
