...
// AllowedDokkuCommands returns the set of dokku top-level commands the
// dashboard is willing to forward via the command runner UI. This is a hard
// allow-list — anything not listed is rejected before reaching the shell.
//
// The list mirrors the safe subset of `dokku --help` plus the subcommands
// most often used in day-to-day tenant operations.
func AllowedDokkuCommands() []string {
	return []string{
		"apps:list", "apps:report", "apps:create", "apps:destroy", "apps:rename",
		"ps:report", "ps:start", "ps:stop", "ps:restart", "ps:rebuild",
		"ps:scale", "ps:inspect",
		"logs", "events",
		"config", "config:show", "config:set", "config:unset",
		"domains", "domains:report", "domains:add", "domains:remove",
		"domains:set", "domains:enable", "domains:disable",
		"ports", "ports:report", "ports:add", "ports:set", "ports:remove",
		"proxy:report", "proxy:set", "proxy:enable", "proxy:disable",
		"network:report", "network:set",
		"checks:report", "checks:enable", "checks:disable", "checks:run", "checks:skip",
		"git:report", "git:from-image", "git:set", "git:sync",
		"plugin:list",
		"version", "report",
		"letsencrypt:list", "letsencrypt:active",
		"resource:report", "resource:limit",
		"storage:list", "storage:mount", "storage:unmount", "storage:report",
		"nginx:show-config", "nginx:validate-config",
	}
}

// IsAllowedDokkuCommand returns true if cmd's first token is in the allow-list.
func IsAllowedDokkuCommand(cmd string) bool {
	cmd = strings.TrimSpace(cmd)
	if cmd == "" {
		return false
	}
	head := strings.Fields(cmd)[0]
	for _, a := range AllowedDokkuCommands() {
		if a == head {
			return true
		}
	}
...
// RunDokku streams the output of an arbitrary (allow-listed) dokku command to
// w as it is produced. argv must already be parsed by the caller; this method
// re-validates the first token against the allow-list as a defence in depth.
func (c *Client) RunDokku(ctx context.Context, w io.Writer, argv []string) error {
	if len(argv) == 0 {
		return fmt.Errorf("empty command")
	}
	if !IsAllowedDokkuCommand(argv[0]) {
		return fmt.Errorf("command %q is not on the allow-list", argv[0])
	}
	full := append([]string{"exec", "-i", c.dokkuName, "dokku"}, argv...)
